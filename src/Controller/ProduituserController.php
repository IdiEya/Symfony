...
public function rechercheProduits(Request $request, ProduitRepository $produitRepo): Response
{
    $nom = $request->query->get('nom');
    $reference = $request->query->get('reference');
    $categorie = $request->query->get('categorie');

    $produits = $produitRepo->findByFilters($nom, $reference, $categorie);
...
public function autocomplete(Request $request, ProduitRepository $repo): JsonResponse
{
    ...
    $produits = $repo->searchByTerm($term); // à implémenter

    $results = [];
    foreach ($produits as $produit) {
        $results[] = ['label' => $produit->getNom(), 'value' => $produit->getNom()];
        $results[] = ['label' => $produit->getRef(), 'value' => $produit->getRef()];
        $results[] = ['label' => $produit->getCategorie(), 'value' => $produit->getCategorie()];
    }
