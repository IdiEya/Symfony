...
final class ProduitsController extends AbstractController
{
    ...
    public function new(Request $request, EntityManagerInterface $entityManager, UserRepository $userRepository, SmsNotifier $smsNotifier): Response
    {
        $produit = new Produit();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            // Envoi SMS à tous les utilisateurs après ajout du produit
            $this->sendNewProductNotifications($produit, $userRepository, $smsNotifier);
    ...
    private function sendNewProductNotifications(Produit $produit, UserRepository $userRepository, SmsNotifier $smsNotifier): void
    {
        $message = sprintf(
    ...
    #[Route('/recherche-produits', name: 'recherche_produits')]
    public function rechercher(Request $request, ProduitRepository $produitRepository): Response
    {
        $query = $request->query->get('q');
        $produits = $produitRepository->findByNomLike($query);
        return $this->render('produits/_table.html.twig', [
            'produits' => $produits,
        ]);
    }

    #[Route('/autocomplete', name: 'autocomplete')]
    public function autocomplete(Request $request, ProduitRepository $repo): JsonResponse
    {
        $term = $request->query->get('term');
        $produits = $repo->searchByTerm($term);

        $results = [];
        foreach ($produits as $produit) {
            $results[] = ['label' => $produit->getNom(), 'value' => $produit->getNom()];
        }
